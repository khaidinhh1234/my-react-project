...
function App() {
  ...
  const AddSanPham = async (sp: any) => {
    try {
      const data: any = await addSanpham(sp);
      console.log(data);
  ...
  const deleteSanPham = async (id: any) => {
    const confirm = window.confirm("Bạn có chăc chắn muốn xóa không !");
    if (confirm)
      try {
        const data: any = await removeSanphamById(id);
        // console.log(data)
  ...
  const EditSanPham = async (sp: any) => {
    try {
  ...
  const DeleteLoaiSanPham = async (id: any) => {
  ...
  const AddLoaiSanPham = async (loaisp: any) => {
    try {
  ...
  const EditLoaiSanPham = async (loaisp: any) => {
    try {
      const data = await updateLoaiSanPham(loaisp);
      // console.log(data)
      toast.success("Cập Nhật Phim Thành Công");
      setLoaisp(
        loaisps.map((item: any) => (item.id === loaisp.id ? loaisp : item))
      );
    } catch (error) {
      toast.error("Lỗi Cập Nhật");
    }
